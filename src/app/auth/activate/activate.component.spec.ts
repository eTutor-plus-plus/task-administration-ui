...
describe('ActivateComponent', () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        provideRouter([
          ...
          {path: 'auth', children: [{path: 'login', component: ActivateComponent}]}
        ]),
        MessageService,
        {provide: API_URL, useValue: 'http://localhost'},
        {provide: AuthService, useValue: {isAuthenticated: () => userAuthenticated}},
        {provide: AccountService, useValue: {activateAccount: (token: string, pwd: string, conf: string) => Promise.resolve()}}
      ]
    }).compileComponents();
  ...
  it('should redirect if user is authenticated', async () => {
    ...
    await fixture.ngZone?.run(async () => await router.navigate(['activate']));
  ...
  it('should redirect if no token is set in the query', async () => {
    // Arrange
    await fixture.ngZone?.run(async () => await router.navigateByUrl('/activate'));
  ...
  it('should not redirect if a token is set in the query', async () => {
    // Arrange
    await fixture.ngZone?.run(async () => await router.navigateByUrl('/activate?token=1234'));
    ...
    // Assert
    expect(router.routerState.snapshot.url).toBe('/activate?token=1234');
  });
  ...
  it('should show error message on failed submit', async () => {
    // Arrange
    TestBed.inject(AccountService).activateAccount = (token: string, pwd: string, conf: string) => Promise.reject('This is an error.');
    await fixture.ngZone?.run(async () => await router.navigateByUrl('/activate?token=1234'));
    component.ngOnInit();
    component.form.patchValue({password: '123456', passwordConfirmation: '123456'});
    fixture.detectChanges();
